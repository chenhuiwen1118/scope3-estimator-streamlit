...
INPUT_FILE = DATA_DIR / "tier2_unified.csv"
OUTPUT_EMBEDDINGS = DATA_DIR / "tier2_embeddings.npy"
OUTPUT_METADATA = DATA_DIR / "tier2_metadata.csv"

print("=" * 70)
print("🚀 Tier 2 Embedding 向量生成")
print("=" * 70)
...
def main():
    ...
    print("   相似度測試:")
    # 動態選擇欄位名稱（支援新舊 schema）
    name_col = 'category_name_en' if 'category_name_en' in df.columns else 'name'

    for i, j in test_pairs:
        sim = cosine_similarity(embeddings[i], embeddings[j])
        name_i = df.iloc[i][name_col][:30]
        name_j = df.iloc[j][name_col][:30]
        print(f"      [{i}] {name_i}... vs")
        print(f"      [{j}] {name_j}...")
        print(f"      → 相似度: {sim:.4f}")
    ...
    print(f"   向量維度: {embeddings.shape[1]}")
    print(f"   資料來源:")
    # 動態選擇來源欄位（支援新舊 schema）
    source_col = 'source_database' if 'source_database' in df.columns else 'source'
    for source, count in df[source_col].value_counts().head(10).items():
        print(f"      - {source}: {count:,}")

    print("\n" + "=" * 70)
    print("✅ Tier 2 Embedding 向量生成完成！")
    print("=" * 70)
    print(f"\n📦 生成檔案:")
